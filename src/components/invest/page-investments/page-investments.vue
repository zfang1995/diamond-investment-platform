  ...
  <div>
    ...
    <div class="sorting-bar">
      <div @click="sortParagraphsBy('start_date')">
        <span>最新</span><span :class="paragraphsOrder === 'start_date-asc'?'image-upWardArrow':'image-downWardArrow'"></span>
      </div>
      <div @click="sortParagraphsBy('total_amount')">
        <span>价格</span><span :class="paragraphsOrder === 'total_amount-asc'?'image-upWardArrow':'image-downWardArrow'"></span>
      </div>
      <div @click="sortParagraphsBy('expected_profit')">
        <span>利润</span><span :class="paragraphsOrder === 'expected_profit-asc'?'image-upWardArrow':'image-downWardArrow'"></span>
      </div>
      <!-- <div>
        <span class="image-crumbsNavButton"></span><span></span>
      </div> -->
    </div>
...
export default {
  ...
  methods: {
    ...
    initializeDataMap (params) {
      api.getInvestmentsInfo(params).then((res) => {
    ...
    sortParagraphsBy (basis) {
      ...
      this.initializeDataMap({type: 'single', order: this.paragraphsOrder})
    }
  },
  created () {
    this.initializeDataMap({type: 'single'})
  }
}
</script>
...
.sorting-bar {
  ...
  & .image-downWardArrow, & .image-upWardArrow {
    width: px2rem(50);
    height: px2rem(40);
